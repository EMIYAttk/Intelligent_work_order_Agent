...
class TicketOutput(BaseModel):
    """IT工单结构化输出"""
    fault_category: Literal["hardware_printer", "hardware_network", "software_os", "software_account"] = Field(
        description="故障分类，必须从指定枚举中选择"
    )
    solution: str = Field(
        description="基于技能加载和知识库检索整理后得出的处理步骤，清晰列出"
    )
    ...
    confidence: Optional[float] = Field(
        default=0.8,
        description="模型对解决方案的置信度，范围0-1，这个数值可以基于知识库检索到的文档和用户输入的距离来计算。",
        ge=0, le=1
    )
local_tools = {
...
- 如果返回为空，请回复：“未在知识库中找到直接解决方案，建议您联系IT支持团队（分机号1234）。”

【注意事项】
- 不要跳过 `load_skill` 直接使用 `retrieve_knowledge`，因为 `load_skill` 会激活该技能所需的完整工具集或上下文。
- 如果用户的问题涉及多个技能（例如“打印机卡纸且电脑蓝屏”），优先处理最紧急或用户首先提到的故障，完成后再处理下一个。如果用户提到的故障少于3个，可以考虑一次性处理完。
...
structuring_agent = create_agent(
    ...
    tools=[],     # 不需要任何工具
    system_prompt="""
你是一个工单信息提取助手。你的任务是从用户的服务对话中提取关键信息，并按照指定的 JSON 格式输出。
...
async def create_skills_based_agent(query:str):
    ...
    structured_res = structuring_agent.invoke({"messages": [{"role": "user", "content": text}]})
