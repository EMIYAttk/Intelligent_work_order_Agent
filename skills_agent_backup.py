...
class SkillMiddleware(AgentMiddleware):
    ...
    def __init__(self, all_tools: Dict[str, List[BaseTool]]):
        ...
        self._skills_list_added = False   # 标志位
        # 优化缓存机制
    ...
    async def awrap_model_call(
        ...
        """异步方法： 异步中间中间件"""
        # ========= 1. 首次调用时，追加技能清单到系统消息 =========
        if not self._skills_list_added:
            self._skills_list_added = True
            # 构建技能清单（name + description）
            skills_desc = "\n".join([f"- {skill['name']}: {skill['description']}" for skill in SKILLS])
            skills_prompt = f"\n\n## 可用技能\n{skills_desc}\n使用 `load_skill` 工具加载技能后，你将获得详细指令和对应工具。"
            # 获取当前系统消息
            current_system = getattr(request, 'system_message', None)
            if current_system:
                new_content = current_system.content + skills_prompt
                new_system = SystemMessage(content=new_content)
            else:
                new_system = SystemMessage(content=skills_prompt)
            # 打印验证
            print("=" * 50)
            print("首次调用：技能清单已添加到系统消息")
            print(new_system.content)
            print("=" * 50)
        ...
        try:
            ...
            for skill in current_skills:
                ...
                skill_tools = self.all_tools.get(skill, [])
                if skill_tools:
                    # 为当前技能生成唯一标识
...
async def load_skill(skill_name: str, tool_call_id: Annotated[str, InjectedToolCallId]) -> Command:
    ...
    for skill in SKILLS:
        if skill["name"] == skill_name:
            ...
            full_content = skill.get("content", "").strip()
            skill_message = SystemMessage(content=f"## 技能 {skill_name} 详细说明\n{full_content}")
            return Command(
                update={
                    "messages": [ToolMessage(content=f"技能 '{skill_name}' 已加载,请根据详细说明使用对应的工具。", tool_call_id=tool_call_id),
                                 skill_message],
...
# 4. 按照技能说明中的指导使用合适的工具,获取数据后，不要再对同一技能进行多次工具调用，除非数据不完整。
# 在完成以上步骤并获得各工具的返回结果后，基于所有工具输出给出最终计划。未完成任一步骤不得输出最终答案，也不要一直重复执行某一步骤!
...
#     print(f"\n技能: {skill['name']}")
#     print(f"  描述: {skill['description'][:80]}..." if len(skill['description']) > 80 else f"  描述: {skill['description']}")
#     print(f"  工具数量: {len(skill['tools'])} ({', '.join(tool_names)})")
#     print(f"  内容预览: {skill['content'][:100]}...")
    
    
# math_skill = next(s for s in SKILLS if s['name'] == 'math')
# tool = math_skill['tools'][0]  # 假设第一个工具是 calculate_budget

# # 模拟调用
# test_activities = ["故宫", "颐和园"]
# result = tool.invoke({"activities": test_activities})  # 或 tool.run(activities=test_activities)
# print("调用结果:", result)
...
#         "messages": [{"role": "user", "content": "请帮我查询天气信息，然后规划上海周末旅行,最后计算本次旅行的预算！"}]
#     })
# )
...
# if "usage_metadata" in res["messages"]:
#     print("run-level usage_metadata:", res["usage_metadata"])
